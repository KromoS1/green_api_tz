...
const useCreateApiReceiveNotification = (state: AppStateType | null) => {
  ...
  return {
    receiveNotification: () => instance.get<ReceiveNotifyType>(`/waInstance${state?.idInstance}/receiveNotification/${state?.apiTokenInstance}`).then((res) => res.data),
    deleteNotification: (receiptId: number) => instance.delete<DeleteNotifyType>(`/waInstance${state?.idInstance}/deleteNotification/${state?.apiTokenInstance}/${receiptId}`)
      .then((res) => res.data),
  };
}
